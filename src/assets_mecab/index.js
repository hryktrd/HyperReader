...
var setStatus = function(text) {
  ...
  //        }
    ...
    // add
    var m = text.match(/Downloading[^(]+\((\d+)\/(\d+)\)/);
    if(m){
      //console.log('matched: '+ m);
      var progress = new ProgressEvent('mecab_loading', {'lengthComputable': false, 'loaded': m[1], 'total': m[2]});
      window.dispatchEvent(progress);
    }
    if(!text){
      //var s = document.getElementById("status");
      //s.innerHTML = "true";
      var event = new Event('mecab_loaded');
